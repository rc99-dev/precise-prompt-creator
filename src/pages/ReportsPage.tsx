...
import { BarChart3, TrendingUp, Clock, DollarSign, Loader2 } from "lucide-react";
...
const suppliersConfig: ChartConfig = {
  total: { label: "Total (R$)", color: "hsl(var(--primary))" },
};

const approvalConfig: ChartConfig = {
  dias: { label: "Dias", color: "hsl(var(--primary))" },
};
...
export default function ReportsPage() {
  ...
  const [supplierRanking, setSupplierRanking] = useState<SupplierRank[]>([]);
  const [approvalTimes, setApprovalTimes] = useState<ApprovalTime[]>([]);
  const [savings, setSavings] = useState({ bestPrice: 0, singleSupplier: 0, economy: 0 });
  const [totals, setTotals] = useState({ orders: 0, totalValue: 0, avgApproval: 0 });
  ...
  async function fetchData() {
    ...
    const [ordersRes, itemsRes, approvalRes] = await Promise.all([
      supabase
        ...
        .select("id, created_at, total, status, approved_at, modo")
        .gte("created_at", startDate)
        .lt("created_at", endDate)
        .not("status", "eq", "cancelado"),
      supabase
        .from("purchase_order_items")
        .select("order_id, supplier_id, subtotal, suppliers(razao_social)")
    ...
    approvals.forEach((a) => {
      ...
      if (order) {
        ...
        const approved = new Date(a.created_at).getTime();
        const days = Math.max(0, (approved - created) / (1000 * 60 * 60 * 24));
    ...
    setApprovalTimes(
      Array.from(approvalMonths.entries()).map(([m, d]) => ({
        month: MONTHS[m],
        dias: d.count > 0 ? Math.round((d.totalDays / d.count) * 10) / 10 : 0,
      }))
    );

    // Savings estimate (best price vs single supplier orders)
    const bestPriceOrders = orders.filter((o) => o.modo === "melhor_preco");
    const singleOrders = orders.filter((o) => o.modo === "melhor_fornecedor");
    const bestPriceTotal = bestPriceOrders.reduce((s, o) => s + (o.total || 0), 0);
    const singleTotal = singleOrders.reduce((s, o) => s + (o.total || 0), 0);
    setSavings({
      bestPrice: bestPriceTotal,
      singleSupplier: singleTotal,
      economy: Math.max(0, singleTotal - bestPriceTotal),
    });
    ...
    const allApprovalDays = approvals
      .map((a) => {
        ...
        return (new Date(a.created_at).getTime() - new Date(order.created_at).getTime()) / (1000 * 60 * 60 * 24);
      })
      .filter((d): d is number => d !== null);
    const avgApproval = allApprovalDays.length > 0
      ? Math.round((allApprovalDays.reduce((s, d) => s + d, 0) / allApprovalDays.length) * 10) / 10
      : 0;

    setTotals({ orders: orders.length, totalValue, avgApproval });
    setLoading(false);
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-base">Total de Compras por Mês</CardTitle>
          </CardHeader>
          <CardContent>
                  ...
                  content={
                    <ChartTooltipContent
                      formatter={(value, name) =>
                        name === "total" ? formatCurrency(Number(value)) : String(value)
                      }
                    />
                  }
                />
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-base">Ranking de Fornecedores (Top 10)</CardTitle>
          </CardHeader>
          <CardContent>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-base">Tempo Médio de Aprovação (dias)</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-base">Economia por Estratégia</CardTitle>
          </CardHeader>
