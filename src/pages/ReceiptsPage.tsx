...
import { formatCurrency, formatDate, statusLabels } from "@/lib/helpers";
...
export default function ReceiptsPage() {
  const { user } = useAuth();
  const [orders, setOrders] = useState<EmittedOrder[]>([]);
  ...
  const fetchOrders = async () => {
    const { data } = await supabase.from('purchase_orders').select('id, numero, status, total, created_at')
      .in('status', ['emitido']).order('created_at', { ascending: false });
    setOrders((data || []) as EmittedOrder[]);
  };
